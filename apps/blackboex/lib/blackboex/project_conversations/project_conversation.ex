defmodule Blackboex.ProjectConversations.ProjectConversation do
  @moduledoc """
  Container for AI chat runs scoped to a single Project. At most one
  conversation per project can be `active` at a time; older threads are
  archived. Mirrors the shape of `Blackboex.FlowConversations` and
  `Blackboex.PageConversations` so the project-level agent reuses the same
  chat experience.
  """
  ...
  schema "project_conversations" do
    ...
    has_many :runs, Blackboex.ProjectConversations.ProjectRun, foreign_key: :conversation_id
  ...
  def changeset(conversation, attrs) do
    conversation
    |> cast(attrs, [:project_id, :organization_id, :status])
    |> validate_required([:project_id, :organization_id, :status])
    |> validate_inclusion(:status, @valid_statuses)
    |> unique_constraint(:project_id, name: :project_conversations_unique_active)
