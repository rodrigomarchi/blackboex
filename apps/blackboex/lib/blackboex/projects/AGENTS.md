...
| `provision_sample_workspace/2` | `(organization, user)` | `{:ok, %{project, membership}}` | Creates the managed `"Examples"` project and samples |
...
- Every new organization starts with exactly one managed sample project named `"Examples"`.
