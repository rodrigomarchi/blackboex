defmodule Blackboex.Apis.DashboardQueriesTest do
  ...
  describe "usage_metrics/2 with {:org, id}" do
    test "returns zeros for org with no usage", %{org: org} do
      result = DashboardQueries.usage_metrics({:org, org.id}, "30d")

      assert result.totals.api_invocations == 0
      assert result.totals.llm_generations == 0
      assert result.totals.tokens_input == 0
      assert result.totals.tokens_output == 0
      assert result.totals.llm_cost_cents == 0
      assert result.daily_series == []
      assert result.by_event_type == []
    end

    test "aggregates daily usage and event types across projects", %{org: org, user: user} do
      default_project = Blackboex.Projects.get_default_project(org.id)
      ...
      assert result.totals.api_invocations == 15
      assert result.totals.llm_generations == 3
      assert result.totals.tokens_input == 150
      assert result.totals.tokens_output == 75
      assert result.totals.llm_cost_cents == 35
      assert length(result.daily_series) == 2

      types = result.by_event_type |> Enum.map(& &1.event_type) |> Enum.sort()
      assert types == ["api_invocation", "llm_generation"]
    end

    test "respects the period window", %{org: org} do
      default_project = Blackboex.Projects.get_default_project(org.id)
      ...
      result = DashboardQueries.usage_metrics({:org, org.id}, "24h")

      assert result.totals.api_invocations == 0
    end
  end

  describe "usage_metrics/2 with {:project, id}" do
    test "excludes other projects in the same org", %{org: org, user: user} do
      default_project = Blackboex.Projects.get_default_project(org.id)
      ...
      result = DashboardQueries.usage_metrics({:project, default_project.id}, "30d")

      assert result.totals.api_invocations == 7
      assert result.totals.llm_cost_cents == 14
      assert length(result.daily_series) == 1

      types = Enum.map(result.by_event_type, & &1.event_type)
      assert types == ["api_invocation"]
    end
  end
