defmodule Blackboex.PageAgentIntegrationTest do
  ...
  Paragraph generated by the agent.
  ~~~
  ...
  test "end-to-end: start → kickoff → session → chain → page.content updated",
    ...
    assert {:ok, _job} = PageAgent.start(page, scope, "write intro")
    ...
    assert content =~ "# AI Title"
    assert summary =~ "initial draft"

    updated = Pages.get_page(page.project_id, page.id)
    assert updated.content =~ "# AI Title"
