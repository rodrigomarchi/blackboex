...
| `ProjectAgentLive.Index` | app | Project Agent UI under `/orgs/:org_slug/projects/:project_slug/agent`. Gated by `Blackboex.Features.project_agent_enabled?/1` — flag-off redirects to project root with a flash. Empty state submits a single user message via `Blackboex.ProjectAgent.start_planning/3`. Subscribes to `"project_plan:project:#{project.id}"` BEFORE fetching the active plan (race-free per `feedback_streaming_debug.md`); on `{:plan_drafted, plan}` also subscribes to `"project_plan:#{plan.id}"` and reseeds `stream(:tasks, …)`. On `{:project_task_completed, %{task_id, …}}` re-fetches the task and `stream_insert/3`s it. Approval delegates to `Blackboex.ProjectAgent.approve_and_run/3`; concurrent-active-plan, invalid-markdown-edit, and already-terminal errors map to flashes / inline violations. Renders `PlanEditorComponent` (markdown editor for `:draft`, read-only otherwise) and `TaskRowComponent` per task. Key assigns: `project`, `plan` (or nil), `planning`, `violations`, `message_input`, stream `:tasks`. | `submit_message`, `message_change`, `approve_plan` |
