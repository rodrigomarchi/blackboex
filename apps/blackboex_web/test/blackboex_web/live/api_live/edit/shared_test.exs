  ...
  setup %{user: user} do
    {:ok, %{organization: org}} =
      Blackboex.Organizations.create_organization(user, %{
        name: "Shared Test Org #{System.unique_integer([:positive])}",
        slug: "sharedorg-#{System.unique_integer([:positive])}"
      })

    {:ok, api} =
      Apis.create_api(%{
        name: "Shared Test API",
        slug: "shared-test-#{System.unique_integer([:positive])}",
