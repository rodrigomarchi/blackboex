...
> - Trabalho async em LiveView DEVE usar `Task.async` + `handle_info({ref, result})` + `handle_info({:DOWN, ...})`. NUNCA `send(self(), :do_work)` — bloqueia o processo LiveView
...
- [x] Adicionar `live_monaco_editor ~> 0.2` ao `mix.exs` do app web
- [x] Configurar assets:
...
- [x] Escrever teste LiveView para componente `CodeEditor`:
...
- [x] Implementar componente `BlackboexWeb.Components.CodeEditor`:
...
- [x] Verificar: Monaco renderiza no browser

## 2. Tela de Edicao

Ref: discovery/04 secao 2.1 (layout, abas, permissoes)

- [x] Escrever testes LiveView para `BlackboexWeb.ApiLive.Edit`:
...
- [x] Implementar LiveView `BlackboexWeb.ApiLive.Edit`:
...
- [x] Aba "Info": nome, slug, descricao, template type, URL (se compilada)
- [x] Aba "Teste Rapido": URL + selector method + body textarea + botao enviar + resposta
- [x] Verificar: navegacao da lista de APIs para o editor funciona
...
- [x] Implementar acoes no LiveView:
...
- [x] Verificar: editar -> salvar -> compilar funciona sem reload

## 4. Sistema de Versoes

Ref: discovery/04 secoes 3.1–3.2 (schema, auto-increment, source enum, llm_response)

- [x] Escrever testes para schema `Blackboex.Apis.ApiVersion`:
...
- [x] Criar migration para tabela `api_versions`:
...
- [x] Implementar schema `Blackboex.Apis.ApiVersion`
- [x] Escrever testes para contexto:
...
- [x] Implementar funcoes de versionamento no contexto `Blackboex.Apis`
- [x] Clarificar relacao `apis.source_code` vs `api_versions.code`: `apis.source_code` e cache da versao atual, atualizado na mesma transacao que a criacao da versao
- [x] Verificar: cada save cria versao com numero incrementado
...
- [x] Escrever testes para `Blackboex.Apis.DiffEngine`:
...
- [x] Implementar `DiffEngine`: dividir codigo em linhas com `String.split(code, "\n")` e aplicar `List.myers_difference/2`. Ref: discovery/04 secao 4.1
- [x] Salvar `diff_summary` ao criar versao (comparando com anterior)
- [x] Escrever teste LiveView: botao "Ver diff" abre diff viewer
- [x] Implementar componente `DiffViewer` com Monaco diff editor (JS hook)
- [x] Verificar: diffs calculados e exibidos corretamente

## 6. Historico de Versoes na UI

Ref: discovery/04 secao 5.1 (listagem, navegacao, diff_summary)

- [x] Escrever testes LiveView para aba "Versoes":
...
- [x] Implementar aba "Versoes" no painel direito
- [x] Verificar: historico navega entre versoes
...
- [x] Implementar `rollback_to_version/2` no contexto
...
- [x] Implementar rollback na UI com dialog de confirmacao
- [x] Verificar: rollback funciona end-to-end

## 8. Hot Reload

Ref: discovery/04 secao 7.1 (BEAM code loading, soft_purge, grace period)

- [x] Escrever testes de integracao:
...
- [x] Implementar hot reload no `Compiler`:
...
- [x] Verificar: recompilar API ativa nao causa downtime
...
- [x] Testes: schema, contexto, LiveView, integracao hot reload

---

## Criterios de Aceitacao

- [x] Monaco Editor renderiza com syntax highlighting Elixir
- [x] Editar e salvar funciona (toast de confirmacao)
- [x] Compilar mostra sucesso ou erros formatados com linha
- [x] Cada save cria nova versao no historico (1-based numbering)
- [x] Historico mostra versoes com metadata e diff_summary
- [x] Clicar em versao antiga mostra codigo (read-only)
- [x] Diff visual mostra mudancas entre versoes
- [x] Rollback cria nova versao, recompila, e modulo fica carregado com status "compiled"
- [x] Hot reload funciona sem downtime
- [x] `make precommit` passa
- [x] 100% TDD
