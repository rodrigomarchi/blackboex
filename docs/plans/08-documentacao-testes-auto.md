...
- [x] Adicionar `open_api_spex ~> 3.22` ao `mix.exs` do app **web** (`blackboex_web`) para SwaggerUI plug
- [x] Adicionar `{:ex_json_schema, "~> 0.10"}` ao `mix.exs` do app domain para validacao de respostas (contract testing)
- [x] Verificar: `mix deps.get` compila sem erros

## 2. Migrations (antes das implementacoes)

Ref: discovery/05 secoes 3, 5 (schema de docs e testes)

- [x] Criar migration para adicionar campo `documentation_md` (text, nullable) na tabela `apis`
- [x] Criar migration para tabela `test_suites`:
  - `id` (UUID), `api_id`, `version_number`, `test_code`, `status`,
    `results` (jsonb), `total_tests`, `passed_tests`, `failed_tests`, `duration_ms`,
    `inserted_at`, `updated_at`
- [x] Verificar: migrations rodam sem erros

## 3. Geracao de OpenAPI Spec

Ref: discovery/06 secao 6 (OpenAPI generation), discovery/05 secao 6

- [x] Escrever testes para `Blackboex.Docs.OpenApiGenerator`:
...
- [x] Implementar `Blackboex.Docs.OpenApiGenerator`
- [x] Verificar: specs geradas sao validas
...
- [x] Escrever testes para endpoint de spec:
...
- [x] Implementar endpoint que serve spec JSON: `/api/:username/:slug/openapi.json`
- [x] Implementar endpoint YAML: `/api/:username/:slug/openapi.yaml`
- [x] Implementar Swagger UI usando `open_api_spex` SwaggerUI plug:
  - Tema escuro
- [x] "Try it out" funciona (requests direto do Swagger UI)
- [x] Adicionar secao de code snippets na pagina de documentacao:
  - cURL, Python (requests), JavaScript (fetch), Elixir (HTTPoison)
  - Ref: discovery/05 secao 8
- [x] Link "Documentacao" na pagina publica da API
- [x] Verificar: Swagger UI renderiza e "Try it out" funciona

## 5. Geracao de Testes pelo LLM

Ref: discovery/05 secoes 2.2, 2.3 (prompt template, retry loop)

- [x] Escrever testes para `Blackboex.Testing.TestGenerator`:
...
- [x] Definir prompt template para geracao de testes:
...
- [x] Implementar `Blackboex.Testing.TestGenerator`:
...
- [x] Implementar `Blackboex.Testing.TestRunner`:
...
- [x] Verificar: testes executam e resultados capturados
...
- [x] Escrever teste LiveView: Aba "Testes" no editor mostra botao "Gerar Testes"
- [x] Implementar botao "Gerar Testes" que chama `TestGenerator.generate_tests/1`
- [x] Apos gerar, Monaco editor mostra codigo dos testes (editavel)

### 8b. Botao "Executar Testes"
- [x] Escrever teste LiveView: Botao "Executar Testes" visivel apos testes gerados
- [x] Implementar botao que chama `TestRunner.run/1`

### 8c. Lista de Resultados
- [x] Escrever teste LiveView: Resultados com icone verde/vermelho por teste
- [x] Escrever teste LiveView: Teste falho expandivel mostra assertion, esperado vs recebido
- [x] Implementar lista de resultados com expand/collapse

### 8d. Badge de Status
- [x] Escrever teste LiveView: Badge no header "5/5 testes passando"
- [x] Implementar badge com contagem

### 8e. Historico de Execucoes
- [x] Escrever teste LiveView: Historico mostra ultimas 10 execucoes
- [x] Implementar historico de execucoes

### 8f. Regenerar Testes
- [x] Escrever teste LiveView: Botao "Regenerar Testes" disponivel
- [x] Implementar botao que chama `TestGenerator` novamente

- [x] Verificar: fluxo gerar -> editar -> executar -> ver resultados
...
- [x] Escrever testes para `Blackboex.Testing.ContractValidator`:
...
- [x] Implementar com `ExJsonSchema.Validator.validate/2`:
...
- [x] Verificar: violacoes detectadas e reportadas
...
- [x] Escrever testes para `Blackboex.Docs.DocGenerator`:
...
- [x] Implementar `Blackboex.Docs.DocGenerator`
- [x] Renderizar doc na pagina publica da API (alem do Swagger UI)
- [x] Verificar: documentacao gerada e exibida
...
- [x] OpenAPI 3.1 spec gerada para cada API publicada (JSON e YAML)
- [x] Swagger UI em `/api/:username/:slug/docs`
- [x] Spec JSON em `/api/:username/:slug/openapi.json`
- [x] "Try it out" funciona no Swagger UI
- [x] Code snippets na pagina de documentacao (cURL, Python, JS, Elixir)
- [x] LLM gera testes ExUnit que compilam e executam (com retry loop ate 3x)
- [x] Resultados de testes no browser com status por teste
- [x] Testes editaveis pelo usuario
- [x] Validacao de contrato via `ExJsonSchema` detecta respostas fora da spec
- [x] DB isolation via Ecto Sandbox durante execucao de testes
- [x] Documentacao Markdown na pagina publica
